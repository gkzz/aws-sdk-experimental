...
describe("run main", () => {
  it("Successful", async () => {
    ec2ClientMock.on(RunInstancesCommand, runInstancesCommandInput).resolves({
      $metadata: {
        httpStatusCode: 200,
      },
    ...
    expect(ec2ClientMock).toHaveReceivedCommandTimes(
      DescribeInstancesCommand,
      0
    );
    expect(ec2ClientMock).toHaveReceivedCommandTimes(
  ...
  it("RunInstancesCommand failed", async () => {
    ec2ClientMock.on(RunInstancesCommand, runInstancesCommandInput).rejects();
  ...
  it("TerminateInstancesCommand failed", async () => {
    ...
    await expect(main()).rejects.toThrow(
      "Failed to terminate the EC2 Instance"
    );
